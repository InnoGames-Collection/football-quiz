const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/TournamentService-BM-k9Jq3.js","assets/supabase-kic3bLQH.js"])))=>i.map(i=>d[i]);
                    ...
                    <div style="background: ${E}; border: 1px solid ${L}; padding: 10px 12px; border-radius: 8px; margin-bottom: 8px; display: flex; flex-direction: column; gap: 4px;">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <div style="display: flex; align-items: center; font-size: var(--fds-font-sm); font-weight: 700; color: var(--fds-text-main);">
                                ${T}
                                <span style="color: var(--fds-gold-primary); margin-right: 8px;">${N}</span> 
                                ${y}
                            </div>
                            ${$}
                        </div>
...
`).map(r=>r.trim()).filter(r=>r.length>0);if(i.length<2){this._statusMessage="❌ CSV must contain a header row and at least 1 data row.",this.render();return}let a=0;const n=i.slice(1);for(const r of n){const o=r.split(",").map(l=>l.trim());if(o.length>=8){const[l,c,d,g,m,b,v,y]=o,u={category:l||"football-history",difficulty:parseInt(c||"1",10),competition_id:l||"football-history",prompt_en:d,options_en:[g,m,b,v],correct_index:parseInt(y||"0",10),is_active:!0};h&&await h.from("questions").insert(u),a++}}this._statusMessage=`✅ Successfully processed & imported ${a} questions!`,this.render()}),e.querySelector("#admin-add-comp-btn")?.addEventListener("click",()=>{const t=e.querySelector("#admin-comp-name"),i=e.querySelector("#admin-comp-badge"),a=e.querySelector("#admin-comp-desc");if(t&&t.value.trim()!==""){const n=t.value.toLowerCase().replace(/\s+/g,"-");k.addCompetition({id:n,name:t.value.trim(),nameEn:t.value.trim(),badge:i.value.trim()||"⚽",description:a.value.trim()||"Custom Competition",color:"#1e3a8a",questionCount:10}),this._audioManager.playClick(),this._statusMessage=`✅ Competition '${t.value.trim()}' Published Successfully!`,this.render()}})}}class H{static _instance=null;static getInstance(){return H._instance||(H._instance=new H),H._instance}async getLeaderboard(e,t="all_time",i=50){if(_.isOnline&&h)try{if(t==="daily"){const a=new Date().toISOString().split("T")[0],{data:n,error:r}=await h.rpc("get_daily_leaderboard",{p_date:a});if(!r&&n&&Array.isArray(n)){const o=n.map((c,d)=>({rank:d+1,userId:c.user_id,username:c.username||"Anonymous Player",avatarUrl:c.avatar_url,eloRating:1200,score:c.score||0,matchesPlayed:1,wins:1})),l=h.auth.getUser?(await h.auth.getUser()).data.user?.id:null;if(l){const c=o.find(d=>d.userId===l);c?(localStorage.setItem("ETHIO_DAILY_RANK",c.rank.toString()),localStorage.setItem("ETHIO_DAILY_SCORE",c.score.toString())):localStorage.removeItem("ETHIO_DAILY_RANK")}return o}}else{const{data:a,error:n}=await h.rpc("get_leaderboard",{p_competition_id:e||null,p_time_range:t,p_limit:i});if(!n&&a&&Array.isArray(a))return a.map(r=>({rank:r.rank,userId:r.user_id,username:r.username||"Anonymous Player",avatarUrl:r.avatar_url,eloRating:r.elo_rating||1200,score:r.score||0,matchesPlayed:r.matches_played||0,wins:r.wins||0}))}}catch(a){console.warn("[LeaderboardService] RPC query failed, returning empty list:",a)}return[]}async getUserRank(e,t){if(!e)return null;try{const a=(await this.getLeaderboard(t)).find(n=>n.userId===e);if(a)return a.rank}catch(i){console.warn("[LeaderboardService] Failed to get user rank:",i)}return null}}class Je{static checkAndShow(e){const t="ETHIO_FOOTBALL_LAST_LOGIN",i=new Date().toISOString().split("T")[0];if(localStorage.getItem(t)===i)return;localStorage.setItem(t,i);const r=(e.profile.streakCount||0)+1;e.updateStreak(r);const o=100+r*25;e.addXp(o)}}class q{static _instance=null;_memoryCache=new Map;_isQuizActive=!1;static getInstance(){return q._instance||(q._instance=new q),q._instance}constructor(){}setQuizActive(e){this._isQuizActive=e,console.log(`[CacheManager] Active quiz status: ${e}`)}get isQuizActive(){return this._isQuizActive}async getOrFetch(e,t,i={}){const a=i.ttlMs??3e5,n=this.get(e);if(n&&!i.forceRefresh&&!this.isStale(e))return n;if(this._isQuizActive&&n)return console.log(`[CacheManager] Quiz active. Returning cached data for key: ${e}`),n;try{const r=await t();return this.set(e,r,a),r}catch(r){if(console.warn(`[CacheManager] Fetch failed for key '${e}'. Falling back to cache if available.`,r),n)return n;throw r}}get(e){if(this._memoryCache.has(e))return this._memoryCache.get(e).data;try{const t=localStorage.getItem(`ETHIO_CACHE_${e}`);if(t){const i=JSON.parse(t);return this._memoryCache.set(e,i),i.data}}catch{}return null}set(e,t,i=300*1e3){const a={data:t,timestamp:Date.now(),ttlMs:i};this._memoryCache.set(e,a);try{localStorage.setItem(`ETHIO_CACHE_${e}`,JSON.stringify(a))}catch{}}isStale(e){const t=this._memoryCache.get(e);if(!t)try{const i=localStorage.getItem(`ETHIO_CACHE_${e}`);if(!i)return!0;const a=JSON.parse(i);return Date.now()-a.timestamp>a.ttlMs}catch{return!0}return Date.now()-t.timestamp>t.ttlMs}invalidate(e){this._memoryCache.delete(e);try{localStorage.removeItem(`ETHIO_CACHE_${e}`)}catch{}}clear(){this._memoryCache.clear();try{Object.keys(localStorage).forEach(e=>{e.startsWith("ETHIO_CACHE_")&&localStorage.removeItem(e)})}catch{}}}class se{static attach(e,t){let i=0,a=0,n=!1;const r=document.createElement("div");r.className="pull-to-refresh-indicator",r.style.cssText=`
        ...
        `,await this._fetchData(),this._bindEvents(),this._renderMessages()}async _fetchData(){const e=O.getInstance();this._currentTab==="announcements"?this._messages=await e.getAnnouncements():this._currentTab==="personal"?this._messages=await e.getPersonalMessages():this._currentTab==="support"&&(this._messages=await e.getSupportTickets())}_renderTab(e,t){const i=O.getInstance().getTotalUnreadCount(),a=e==="personal"?i:0;return`
            <div class="mc-tab ${this._currentTab===e?"mc-tab-active":""}" data-tab="${e}">
                ${t}
                ${a>0?`<div class="mc-tab-badge">${a}</div>`:""}
            </div>
        `}_renderFilterChips(){return[{id:"all",label:"All"},{id:"unread",label:"Unread"},{id:"high-priority",label:"Urgent"},{id:"tournament",label:"Tournaments"},{id:"reward",label:"Rewards"}].map(t=>`
            <div class="filter-chip ${this._currentFilter===t.id?"filter-chip-active":""}" data-filter="${t.id}">
                ${t.label}
            </div>
